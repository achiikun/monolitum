...
abstract class Renderable_Node extends Node implements Active {
    ...
    /**
     * @var null|Renderable_Node|array<Renderable_Node>
     */
    private $childs = null;
     ...
     * @return void
     */
    private function insertIntoArray($element, $idx = null){
        if($idx !== null){
            if($this->childs === null){
                $this->childs = $element;
            }else{
    ...
    /**
     * @param Renderable_Node|mixed $component
     * @return $this
     */
    public function append($component, $idx = null)
    {
        if ($component instanceof Renderable_Node) {
            if ($component->isBuilt() && $component->getParent() !== $this)
                throw new DevPanic("Component has a parent.");
            if ($this->isBuilding())
                $this->insertIntoArray($this->buildChild($component), $idx);
            else
                $this->insertIntoArray($component, $idx);
        }else {
            $this->insertIntoArray($component, $idx);
        }
        return $this;
